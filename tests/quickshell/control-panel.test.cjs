...
test("the fixed Fedora button owns the ownerless Control Panel anchor", () => {
    ...
    assert.match(button, /Quickshell\.iconPath\("fedora-logo-icon", true\)/);
    assert.match(button, /width:\s*Theme\.barIconSize/);
    assert.match(button, /height:\s*Theme\.barIconSize/);
    assert.match(button, /colorization:\s*1/);
    assert.match(button, /colorizationColor:\s*Theme\.barTextHi/,
        "the Fedora mark must use the brighter menubar foreground tone");
