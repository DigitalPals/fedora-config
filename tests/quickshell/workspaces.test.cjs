...
test("occupied workspaces stay visibly distinct from empty slots", () => {
    ...
    assert.match(workspaces,
        /root\.numbered\s*\?\s*\(exists\s*\?\s*Theme\.chipHover\s*:\s*Theme\.chip\)/,
        "numbered slots need separate occupied and empty fills");
    assert.match(workspaces, /exists\s*\?\s*Theme\.wsOccupied\s*:\s*Theme\.dotDim/,
        "dot slots need a dedicated occupied tone");
    assert.match(theme, /readonly property color wsOccupied:/,
